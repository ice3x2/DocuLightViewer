...
- [x] SpecKiwi guarded edit로 `FR-DOC-019 AC-10`에 private `manage_index {operation: rebuild|retry|compact|clear}`의 엄격한 입력·상태 gate·명령별 실패/취소·확인된 clear 의미를 미구현 계획으로 기록했다. [SRS·rollback 독립 검토](../analysis/2026-09-25-s23-srs-review.json)와 [TDD·복구 독립 검토](../analysis/2026-09-25-s23-srs-tdd-review.json)는 실험적 owner maintenance 코드·테스트를 모두 HEAD로 되돌린 docs-only 해시 `175381d337ea84c434b2aedac1144a12a9b6f9816a7d27fe635bec0c8fdc541c`를 안전한 기록으로 판정했다. SpecKiwi validate error 0, 기존 경고 8건이다.
- [ ] #46 전체 완료는 보류한다. S26에서 Settings linked import는 owner/read-only로 연결됐고 S24에서 등록 UI는 제거됐으나, Settings 재구축·재시도·압축·초기화의 짧은 legacy worker/main ledger writer와 contained opened registrar의 legacy adoption, rebuild 완료 semantic callback이 남아 있다. owner Settings 명령은 SRS에만 계획돼 있으며, 실험 구현은 stale retry 거절·compact 기존 검색 소실·확인 없는 clear·동기 VACUUM/backup 지연·파생 reset/requeue 누락으로 안전하지 않아 전부 되돌렸다. 대체 경로·실제 Electron writer audit·S19/S17 복구 증거 전에는 live worker를 삭제하지 않고 #43·#44·#45도 열린 상태로 둔다. S26의 한 번 실패·한 번 통과한 partial-import 재실행도 출시 전 재조사한다.
...
- [x] S24 독립 리뷰는 완료됐고 #47은 닫혔다. #44 S21의 embedding UI 의존성은 해소됐지만 product-main 전체 integration/owner action gate 때문에 #44 자체는 계속 OPEN 상태다.
...
## S25 진행 기록 — 책임별 회귀 하니스 독립 검토 완료

- 시작 SHA `aae49ca39feba3868170c2c0eaea516c64e97d11`, 격리 worktree `DocuLightViewer-r3`. `FR-DOC-019`, `REL-DOC-009`, `DR-DOC-014`, `IR-APP-013`에 Stability 차단 없음.
...
## S23 Settings owner maintenance 보류

- 시작 SHA `0afbd15fa88f5025c1a04d46c2b8ba957994d3c2`. SpecKiwi CLI dry-run 후 `FR-DOC-019 AC-10`을 Settings 전용 strict `manage_index` 4개 operation으로 변경하고 change note를 기록했다. Status/Stability는 유지했으며 SRS validation errors=0, 기존 warnings=8이다.
- [ ] [S23 실험 결과와 독립 검토 차단 사유](../analysis/2026-09-25-s23-cutover-evidence.md): 집중 테스트는 통과했지만 독립 검토가 stale retry 거부, compact ACK 전 committed body 검색 소실, 무확인 clear IPC, owner 동기 maintenance의 query/cancel 정지, rebuild 파생 상태 재설정 누락을 HIGH로 확인했다. 실험적 제품/owner/test 코드는 출발 SHA로 복원했고 `FR-DOC-019 AC-10`의 unchecked 계획 계약만 유지했다. 통과했던 S23 31/S26 25는 #46 완료 증거가 아니다.
- [ ] #46은 계속 OPEN이다. 다음 시도는 위 5개 결함의 semantic RED부터 작성하고, bounded owner 구현·확인된 clear·scoped derived reset·실제 cold Electron writable-open 전수 계측·contained registrar 전환을 마친 뒤 검증한다. 기존 short worker와 S17/S19 adapter는 살아 있다. #43/#45/#44도 각 별도 게이트 전에는 닫지 않는다.
- 복원 sourceHash `175381d337ea84c434b2aedac1144a12a9b6f9816a7d27fe635bec0c8fdc541c`에서 S23 10/S20 21/S25 16과 Settings 계약이 통과했다. 실제 S26은 첫 실행에서 Settings partial import assertion이 한 번 실패했고 별도 재실행에서 25 assertions와 모든 재시작 marker가 통과했다. 이 간헐 실패는 해소된 릴리스 게이트로 세지 않는다.

## S23a 진행 기록 — Settings rebuild/retry owner 전환
...
- [ ] #46 전체 완료는 보류한다. #61의 별도 제품 응답성·checksum gate와 #43/#44/#45의 각 수용 기준이 남아 있다. 구형 short worker/controller는 실제 호환·package-smoke 호출자가 남아 있어 죽은 코드로 단정하여 삭제하지 않았다. #62의 독립 검토와 GitHub 완료 처리를 마친 뒤에도 상위 이슈의 완료율은 별도 gate 전까지 늘리지 않는다.
